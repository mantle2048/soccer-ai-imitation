...
from grf_imitation.infrastructure.datas.batch import _alloc_by_keys_diff, _create_value
from grf_imitation.infrastructure.datas.utils.converter import from_hdf5, to_hdf5

class ReplayBuffer:
    """:class:`~grf_imitation.data.ReplayBuffer` stores data generated from interaction \
    ...
    :param bool ignore_next_obs: whether to store next_obs. Default to False.
    :param bool save_only_last_obs: only save the last obs/next_obs when it has a shape
    ...
    """
    _reserved_keys = ("obs", "act", "rew", "done", "next_obs", "info", "policy")

    def __init__(
        self,
        size: int,
        stack_num: int = 1,
        ignore_next_obs: bool = False,
        ...
        **kwargs: Any,  # otherwise PrioritizedVectorReplayBuffer will cause TypeError
        ...
        self.options: Dict[str, Any] = {
            "stack_num": stack_num,
            "ignore_next_obs": ignore_next_obs,
        ...
        self._save_next_obs = not ignore_next_obs
    ...
    def sample(self, batch_size: int, return_indices: bool=False) -> Tuple[Batch, np.ndarray]:
    ...
    def __getitem__(self, index: Union[slice, int, List[int], np.ndarray]) -> Batch:
        ...
        If stack_num is larger than 1, return the stacked obs and next_obs with shape
        (batch, len, ...).
        """
