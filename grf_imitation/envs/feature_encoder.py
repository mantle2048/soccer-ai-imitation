...
class MyFeatureEncoder:
    ...
    def encode(self, obs):
        ...
        left_team_pos = obs['left_team'][1:].flatten()

        left_team_direct = obs['left_team_direction'][1:].flatten()

        ball_pos = obs['ball']

        ball_direct = obs['ball_direction']

        ball_own_team_idx = obs['ball_owned_team']
        ball_own_team = np.array([0, 0, 0])
        ball_own_team[ball_own_team_idx] = 1

        ball_own_player_idx = obs['ball_owned_player']
        ball_own_player = np.array([0, 0, 0, 0, 0, 0])
        ball_own_player[ball_own_player_idx] = 1

        player_role_idx = obs['left_team_roles'][player_num]
        player_role = np.zeros(10)
        player_role[player_role_idx] = 1

        state_dict = {
            'player_role': player_role,
            'players_pos': left_team_pos,
            'players_direct': left_team_direct,
            'ball_pos': ball_pos,
            'ball_direct': ball_direct,
            'ball_own_team': ball_own_team,
            'ball_own_player': ball_own_player,
        }
        history_state_dict = {}
